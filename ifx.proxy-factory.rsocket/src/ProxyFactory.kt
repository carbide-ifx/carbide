...
class RSocketProxyFactory private constructor(
    ...
    constructor(port: Int, host: String = "localhost") : this(RSocketClientProtocol(host, port))

    private constructor(protocol: IClientProtocol) : this(ProxyFactoryBase(protocol))

    companion object {
        fun forHost(host: IHost): RSocketProxyFactory =
            RSocketProxyFactory(RSocketClientProtocol { "ws://localhost:${host.port(RSOCKET_PROTOCOL_ID)}" })
