...
class LogTest {
    @Test
    fun `structured service tags are readable on the console and retained by actuators`() {
    ...
    @Test
    fun `plain tags are written to the console but not retained by actuators`() {
        val console = RecordingLogWriter()
        val store = ActuatorLogStore()
        val logger = Logger(
            loggerConfigInit(
    ...
    fun `each service retains only its configured capacity in sequence order`() {
        ...
        repeat(5) { index ->
            writer.log(Severity.Info, "message-$index", encodedTag)
        }
